...
public class Prompt implements ShellCommand, Environment {
  ...
  private ShellCommand fallbackCommand;
  ...
  /**
   * Nameless command that is invoked if no other command is available.
   * If it is null, nothing will be invoked and user will be notified that
   * command doesn't exist as well as some fallback option.
   * @param cmd
   */
  public final void registerFallbackCommand(ShellCommand cmd) {
    this.fallbackCommand = cmd;
  }
    ...
    while (true) {
      ...
      if (!commands.containsKey(cmd[0])) { // command not registered
        if (fallbackCommand != null)
          fallbackCommand.executeCommand(env, line);
        else
          env.writeln("Command '%s' doesn't exist ".formatted(cmd[0]));
      }
      else {
        env.setPromptSymbol("Root %s > ".formatted(i++));
